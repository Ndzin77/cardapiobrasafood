...
BEGIN
  ...
  FROM (
    SELECT d.id, d.order_id, d.status, d.assigned_at, d.collected_at, d.delivered_at,
           ...
           o.order_type, o.scheduled_date, o.scheduled_time
    FROM deliveries d
    ...
    WHERE d.driver_id = p_driver_id 
      ...
      AND (
        d.status IN ('assigned', 'collected')
        OR (d.status = 'delivered' AND d.delivered_at >= (CURRENT_DATE AT TIME ZONE 'America/Sao_Paulo'))
      )
  ) sub;
